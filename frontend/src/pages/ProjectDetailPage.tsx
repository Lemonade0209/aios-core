...
export function ProjectDetailPage() {
  ...
  if (!project) return <p className="text-sm text-zinc-600">Loading project...</p>;
    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-sm text-zinc-600">{project.status} · {project.progress}% complete</p>
        </div>
        <div className="flex gap-2">
          <button className="btn-secondary" onClick={() => void summarize()}>Summarize</button>
      ...
      <form className="panel grid gap-4 md:grid-cols-2" onSubmit={onSubmit}>
        ...
        <Field label="Status">
          <select name="status" defaultValue={project.status}>{statuses.map((status) => <option key={status}>{status}</option>)}</select>
        </Field>
        <Field label="Description"><textarea name="description" defaultValue={project.description} /></Field>
        <Field label="Goal"><textarea name="goal" defaultValue={project.goal} /></Field>
        <Field label="Start"><input name="startDate" type="date" defaultValue={project.startDate} /></Field>
        <Field label="Due"><input name="dueDate" type="date" defaultValue={project.dueDate} /></Field>
        <button className="btn-primary md:col-span-2">Save Project</button>
      </form>
      <section className="grid gap-4 lg:grid-cols-3">
        <Related title="Tasks" items={tasks.map((task) => `${task.title} · ${task.status}`)} />
        <Related title="Notes" items={notes.map((note) => note.title)} />
        <Related title="Documents" items={documents.map((doc) => doc.title)} />
      </section>
    </div>
  );
}
...
function Related({ title, items }: { title: string; items: string[] }) {
  return (
    <div className="panel space-y-3">
      <h2 className="font-semibold">{title}</h2>
      {items.length === 0 ? <p className="text-sm text-zinc-500">None linked.</p> : items.map((item) => (
