    ...
    <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
      <form className="panel h-fit space-y-4" onSubmit={onSubmit}>
        <div>
          ...
          <p className="text-sm text-zinc-600">Create and track project goals.</p>
        </div>
        {error && <p className="rounded-md bg-red-50 p-3 text-sm text-red-700">{error}</p>}
        <Field label="Title"><input name="title" required /></Field>
        <Field label="Description"><textarea name="description" /></Field>
        <Field label="Goal"><textarea name="goal" /></Field>
        <Field label="Status">
      ...
      <section className="space-y-3">
        ...
        {!loading && projects.length === 0 ? <EmptyState title="No projects yet" /> : projects.map((project) => (
          <article key={project.id} className="panel">
            <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-600">{project.goal || project.description || 'No description'}</p>
                <p className="mt-2 text-xs uppercase text-zinc-500">{project.status} · {project.dueDate ?? 'No due date'}</p>
              </div>
              <button className="btn-danger" onClick={() => void remove(project.id)}>Delete</button>
            </div>
